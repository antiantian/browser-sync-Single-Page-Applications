<template>
  <div class="tips">
      ...
      <div id="show">
        ...
        <button id="enter4" v-if="isIOS" v-on:click="testiOSClick2">调用iOS-Native</button>
      <button id="enter3" v-if="isAndroid" v-on:click="testClick1">调用android</button>
      <div id="show3"></div>
  </div>
</template>
    ...
    contentgetPageList:function(ID){
      ...
      mounted(){
         ...
         var that = this;
            // 第一连接时初始化bridage
              connectWebViewJavascriptBridge(function(bridge) {
                  ...
                  // 注册一个"functionInJs",
                  bridge.registerHandler("renderInJs", function(data, responseCallback) {
                      //that.renderDetails({title:data,details:'近五场比赛，高尚在场上完全迷失了自己，其三分球更是7投1中，命中率仅为15%。曾经14轮首发的高尚，近来其首发'})
                     alert(data)
                     that.renderDetails(JSON.parse(data))
                     // document.getElementById("show").innerHTML = ("data from Java: = " + data);
        ...
        if(this.isIOS){
           var that = this;
               //iOS使用  开始
            window.setAppType=function(mess){
              that.setAppType(mess)
            }
            window.renderDetails=function(obj){
              that.renderDetails(obj);
            }
         }   
      },
  }
